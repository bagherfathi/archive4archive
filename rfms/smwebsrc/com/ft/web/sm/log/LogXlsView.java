...
/**
 * 实现extremecomponents组件的View接口，在导出日志时使用资源文件中的定义将制定的字段的值转换
 * 
 * @version 1.0
 */
public class LogXlsView implements View {
    private static Log logger = LogFactory.getLog(LogXlsView.class);

    public static final int WIDTH_MULT = 420; // width per char
    ...
    public LogXlsView() {
        if (this.resourceBundle == null) {
            this.resourceBundle = ResourceBundle
                    .getBundle("ActionDefinitionResource");
        }
    }
    ...
    public void body(TableModel model, Column column) {
        ...
        if ("systemCode".equals(alias) || "modelName".equals(alias)
                || "actionCode".equals(alias) || "resultCode".equals(alias)) {
            try {
                String realValue = this.resourceBundle.getString(value);
                value = realValue;
            } catch (Exception ex) {
            }
        }
    ...
    private void fixWidthAndPopulate(HSSFCell cell, double numeric, String value) {
        ...
        if (numeric != NON_NUMERIC) {
            ...
            valWidth = (cell.getRichStringCellValue() + "").length() * WIDTH_MULT;
