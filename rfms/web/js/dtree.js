...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		str += '<input type="checkbox" name="ids" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id +'" class="cx"/>';
	}
		...
		function caBox(formId, regx ) {
			if( document.getElementById( regx ).checked ){
				...
				for (var i=0; i<form.elements.length; i++) {
					...
					if (element.name == "ids" && element.type=='checkbox'){
						// checked next node 
