 ...
 *	@returns boolean
 */
ArrayUtils_contains = function(arr,element) {
 ...
 *	@returns boolean
 */
ArrayUtils_remove = function(arr,element) {
	var result = false;
	var array = [];
	for (var i = 0; i < arr.length; i++) {
		if (arr[i] == element) {
			...
			ArrayUtils_push(array,arr[i]);
		}
	}
	ArrayUtils_clear(arr);
	for (var i = 0; i < array.length; i++) {
		ArrayUtils_push(arr,array[i]);
	}
	array = null;
	return result;
};
 ...
 *	@returns number
 */
ArrayUtils_push = function(arr,element) {
 ...
 *	@returns boolean
 */
ArrayUtils_add = function(arr,element) {
	if (ArrayUtils_contains(arr,element)) {
		return false;
	}
	ArrayUtils_push(arr,element);
	return false;
};
 ...
 *	@returns boolean
 */
ArrayUtils_addAll = function(arr,that) {
	var result = false;
	for (var i = 0; i < that.length; i++) {
		if (ArrayUtils_add(arr,that[i])) {
			result = true;
		}
	}
	return true;
...
org.ditchnet.dom.DomUtils.addClassName = function (target,k) {
	...
	with (org.ditchnet.dom.DomUtils) {
		...
		if (ArrayUtils_contains(classNames,k)) {
			return;
		} else {
			
			ArrayUtils_push(classNames,k);
...
org.ditchnet.dom.DomUtils.removeClassName = function (target,k) {
	with (org.ditchnet.dom.DomUtils) {
		...
		if (!ArrayUtils_contains(classNames,k)) {
			...
			ArrayUtils_remove(classNames,k);
		}
...
org.ditchnet.jsp.TabUtils.switchTab = function (eventSource) {
	with (org.ditchnet.jsp.TabUtils) {
		...
		var values = determineSelectedIndex(eventSource);
		toggleTabVisibility(values[0]);
		setTabCookie(values[0]);
		setSelectedIndexCookie(values[1]);
	}
};
...
org.ditchnet.jsp.TabUtils.setTabCookie = function (selectedIndex) {
	with (org.ditchnet) {
		var name  = "webui.tabs:"+jsp.TabUtils.tabContainer.id;
...
org.ditchnet.jsp.TabUtils.setSelectedIndexCookie = function (selectedPaneId) {
    with (org.ditchnet) {
        var name = jsp.TabUtils.tabContainer.id + ":selectedPaneId";
        var c = new util.Cookie(name,selectedPaneId);
        util.Cookie.addPageCookie(c);
    }
};
	...
	with (org.ditchnet.jsp.TabUtils) {
		for (var i = 0; i < tabs.length; i++) {
			if (tabs[i] == eventSource) {
			    var values = [];
			    values[0] = i;
			    var pos = tabs[i].id.lastIndexOf("-");
			    if (pos > 0) {
			        values[1] = tabs[i].id.substring(0,pos);
			    } else {
			        values[1] = tabs[i].id;
			    }
				return values;
...
org.ditchnet.util.getSelectedPaneId = function ( idName ){
    ...
    for(var i=0;i<tabs.length;i++){
        var pos = tabs[i].id.lastIndexOf("-");   
	    if (pos > 0) {
		    id= tabs[i].id.substring(0,pos);
	    } else {
		    id = tabs[i].id;
	    }
	    var panel = document.getElementById(id);
	    if(panel.className=='ditch-tab-pane' && panel.style.display!='none'){
		    return id;
	    }
    }
}