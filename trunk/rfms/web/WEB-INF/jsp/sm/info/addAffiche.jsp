...
<html:form action="/affiche" onsubmit="return onSubmit(this);">
	...
	<webui:panel title="sysadmin.label.affiche.publish" width="95%" icon="../images/icon_list.gif">
		<webui:formTable>
			<tr>
			...
			<tr>
			    ...
			    <table width="90%" align="center" border="0" cellpadding="0" cellspacing="0">
			      ...
			      <webui:tree 
			        ...
			        <c:if test="${level > 0}">
				        <input class="noborder" type="checkbox" name="orgIds" value="<c:out value='${data.key}'/>" <c:if test="${data.status == 1}">disabled title="组织已被禁止或不可访问"</c:if> <c:if test="${flag}">checked</c:if> />
		...
		<webui:linkButton styleClass="clsButtonFace" href="javascript:onSubmitValidator(afficheForm);" value="sysadmin.button.publish"/>
