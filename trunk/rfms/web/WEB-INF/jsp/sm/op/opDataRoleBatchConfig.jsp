...
<webui:panel title="批量设置操作员业务角色" width="95%" icon="../images/icon_list.gif">
		<webui:formTable>
			<tr>
...
<script>
   ...
   function changeOrg(orgId){
       document.forms.operatorRoleForm.setAttribute('action', '<c:url value="/sm/opDataRole.do?act=batchConfigRole&orgId="/>'+orgId);
   ...
   function postUrl(act){
       if(check()){
          loadOn();
          document.forms.operatorRoleForm.setAttribute('action', '<c:url value="/sm/opDataRole.do?act="/>'+act);
	      document.forms.operatorRoleForm.submit();
	   }
   }
</script>
